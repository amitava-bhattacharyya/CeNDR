...
class template:
    ...
    def fetch_sequence(self, use_template):
        ...
        sample_flag = ""
        if use_template == "REF":
            command = ("samtools faidx {} {}".format(self.reference_file, self.region))
        elif use_template == "ALT" or use_template is None:
            command = ("samtools faidx {} {} | bcftools consensus {}".format(self.reference_file, self.region, self.filename))
        ...
        command = command.format(**locals()); print (command)
        try:
    ...
    def out(self, head):
        ...
        if self.mode == 'snip':
            ...
            st = int(self.region.split(':')[1].split('-')[0])
            for primer_group in self.primers:
                # Fetch variant Count
                variant_count = self.fetch_variant_count(primer_group.amplicon_region, self.output_samples)
                out_indel = out + [variant_count,
                                   self.indel_size,
                                   self.indel_type,
                                   str(primer_group.primer_left),
								   st+primer_group.primer_left.START,
								   st+primer_group.primer_left.END,
                                   str(primer_group.primer_right),
								   st+primer_group.primer_right.START,
								   st+primer_group.primer_right.END,
                                   primer_group.primer_tm,
                                   primer_group.amplicon_length,
...
class primer_vcf(cyvcf2):
    def __init__(self, filename, reference=None, use_template="ALT", polymorphic=True):
        ...
        self.contigs = OrderedDict(list(zip(
            re.compile(r'##contig=<ID=(.*?),').findall(self.raw_header),
            list(map(int, re.compile(r'##contig.*length=(.*?),assembly=.*>').findall(self.raw_header)))
        )))
